...
namespace ApiServiciosMicroformas.resource.clases
{
    public class Repositorio
    {
        ...
        public Models.ResultadoMigraModel consultarDocumento(Models.DatosMigraModel mdlDatosMigra)
        {
            ...
            if (new neUsuSisRolEntEstorg(this._objCoConexionDb).ValidaUsuSisPin(new EnServiciosMicroformas.enUsuSisRolEntEstorg()
            {
                ID_ENT = -1,
                ID_USU = intIdPersona,
            ...
            {
                new neAccesoThread(this._objCoConexionDb.ServerCnx).enviarAccesoAsincrono(new EnServiciosMicroformas.enAcceso()
                {
                    ID_SIS = this._ConfigurationManager.Value.AppSettings.IdSis,
                    COD_FUENTE = "S",
