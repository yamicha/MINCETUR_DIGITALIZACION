                    ...
                    using (OracleDataReader drReader = cmd.ExecuteReader())
                    {
                        ...
                        if (drReader.HasRows)
                        {
                            ...
                            while (drReader.Read())
                            {
                                ...
                                if (!drReader.IsDBNull(intIdFondo)) temp.DES_FONDO = arrResult[intIdFondo].ToString();
                                if (!drReader.IsDBNull(intDesFondo)) temp.ID_SECCION = arrResult[intDesFondo].ToString();
                                if (!drReader.IsDBNull(intIdSeccion)) temp.DES_LARGA_SECCION = arrResult[intIdSeccion].ToString();
                                if (!drReader.IsDBNull(intDesSeccion)) temp.ID_SERIE = arrResult[intDesSeccion].ToString();
                                if (!drReader.IsDBNull(intIdSerie)) temp.DES_SERIE = arrResult[intIdSerie].ToString();
                                if (!drReader.IsDBNull(intDesSerie)) temp.NOM_DOCUMENTO = arrResult[intDesSerie].ToString();
                                if (!drReader.IsDBNull(intNomDocumento)) temp.ANIO = arrResult[intNomDocumento].ToString();
                                if (!drReader.IsDBNull(intDescripcion)) temp.FOLIOS = arrResult[intDescripcion].ToString();
                                if (!drReader.IsDBNull(intAnio)) temp.OBSERVACION = arrResult[intAnio].ToString();
                                if (!drReader.IsDBNull(intFolios)) temp.USU_CREACION = arrResult[intFolios].ToString();
                                if (!drReader.IsDBNull(intObservacion)) temp.STR_FEC_CREACION = arrResult[intObservacion].ToString();
                                if (!drReader.IsDBNull(intUsuariocreacion)) temp.USU_MODIFICACION = arrResult[intUsuariocreacion].ToString();
                                if (!drReader.IsDBNull(intFecCreacion)) temp.STR_FEC_MODIFICACION = arrResult[intFecCreacion].ToString();
                                lista.Add(temp);
                            }
                            drReader.Close();
                        }
